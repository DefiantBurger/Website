<script lang="ts">
  ...
  import { computeSchedulerGraph, detangleCourseOrder } from '../lib/scheduler/logic';
  import type { CourseCatalog, CourseInstance, PrereqEdge, Requirement, ScheduleFile } from '../lib/scheduler/types';
  ...
  let prereqsByCourseId: Record<string, { courseChoices: string[]; canBeConcurrent: boolean }[]> = {};
  ...
  let selectedCourseRequirementGroups: { courseChoices: string[]; canBeConcurrent: boolean }[] = [];
  ...
  function compactDisplayCode(courseName: string): string {
    ...
    const prefix = courseName.slice(0, firstDigitIndex).replace(/[^A-Za-z]/g, '');
    const suffix = courseName.slice(firstDigitIndex).replace(/\s+/g, ' ').trim();
  ...
  function setCourseElement(courseId: string, element: HTMLDivElement | null): void {
    courseElements = {
      ...courseElements,
      [courseId]: element
    };
  ...
  function refreshSemesterCredits(): void {
    ...
    for (const semester of semesterOrder) {
      const total = (columnCourseIds[semester] ?? []).reduce((sum, courseId) => {
  ...
  function updateGraph(useDetangle: boolean): void {
    ...
    if (useDetangle) {
      columnCourseIds = detangleCourseOrder(semesterOrder, columnCourseIds, prerequisiteEdges);
    }
    ...
    const nextSegments = prerequisiteEdges
      .map((edge) => {
        ...
        const relatedToSelection =
          !!selectedCourseId &&
          (selectedCourseId === edge.fromCourseId || selectedCourseId === edge.toCourseId);
        const alwaysHighlightInvalid = edge.status === 'invalid';
        const highlighted = alwaysHighlightInvalid || relatedToSelection;
        const dimmed = !alwaysHighlightInvalid && (!hasSelection || !relatedToSelection);
  ...
  $: nonInvalidEdgeSegments = edgeSegments.filter((segment) => segment.status !== 'invalid');
  $: invalidEdgeSegments = edgeSegments.filter((segment) => segment.status === 'invalid');
  ...
  function defaultCourseCredits(courseName: string): number {
    ...
    if (!item) {
      throw new Error(`Course '${courseName}' not found in course catalog.`);
    }
  ...
  function validateCourseCredits(courseName: string, requestedCredits?: number): number {
    const item = courseCatalog[courseName];
    if (!item) {
      throw new Error(`Course '${courseName}' not found in course catalog.`);
    }
        ...
        : Array.from(
            {
              length: Math.max(
                1,
                (item.max_credits ?? item.min_credits ?? 0) - (item.min_credits ?? 0) + 1
              )
            },
  ...
  function parseCourseEntry(entry: string): { courseName: string; requestedCredits?: number } {
    if (!entry.includes('[')) {
      return { courseName: entry.trim() };
    }

    if (!entry.endsWith(']')) {
      throw new Error(`Malformed course entry '${entry}': bracket not closed.`);
    }
    ...
    if (Number.isNaN(requestedCredits)) {
      throw new Error(`Malformed course entry '${entry}': invalid credit value.`);
    }
  ...
  function resolveCourseNameFromQuery(): string | null {
    ...
    const exactCaseInsensitive = Object.keys(courseCatalog).find(
      (courseName) => courseName.toLowerCase() === fromDisplay.toLowerCase()
    );
  ...
  function addCourseToSemester(): void {
    ...
    if (!courseName) {
      addCourseError = 'Pick a course from the search results before adding.';
      return;
    }
  ...
  function buildScheduleExport(): ScheduleFile {
    ...
    for (const semester of semesterOrder) {
      const entries = (columnCourseIds[semester] ?? []).map((courseId) => {
      ...
      semesters[semester] = entries.filter((entry): entry is string => !!entry);
    }
  ...
  function handleExportJson(): void {
    ...
    dataTransferMessage = 'Exported current schedule to schedule-export.json';
  }

  async function applyImportedSchedule(scheduleFile: ScheduleFile): Promise<void> {
    const importedSemesters = Object.keys(scheduleFile.semesters ?? {});
    const importedOrder = importedSemesters.filter((semester) => semester !== PRECOLLEGE_SEMESTER);

    if (importedOrder.length === 0) {
      throw new Error('Imported schedule must include at least one non-precollege semester.');
    }
    ...
    const nextColumnCourseIds: Record<string, string[]> = Object.fromEntries(
    ...
    for (const [semester, entries] of Object.entries(scheduleFile.semesters ?? {})) {
      if (semester !== PRECOLLEGE_SEMESTER && !importedOrder.includes(semester)) {
        continue;
      }

      for (const entry of entries) {
        const { courseName, requestedCredits } = parseCourseEntry(entry);
        if (!courseCatalog[courseName]) {
          throw new Error(`Course '${courseName}' is not in the loaded course catalog.`);
        }

        const occurrenceIndex = (nextOccurrenceCounts[courseName] ?? 0) + 1;
        nextOccurrenceCounts[courseName] = occurrenceIndex;

        const courseId = `${courseName}__${occurrenceIndex}`;
        const credits = validateCourseCredits(courseName, requestedCredits);
        ...
        nextPrereqsByCourseId[courseId] = buildRequirementsForCourse(courseName);
  ...
  async function handleImportFile(event: Event): Promise<void> {
    ...
    try {
      ...
      if (!parsed || typeof parsed !== 'object' || !parsed.semesters) {
        throw new Error('Invalid schedule JSON format. Expected an object with a semesters field.');
      }

      await applyImportedSchedule(parsed);
      dataTransferMessage = `Imported schedule from ${file.name}`;
    } catch (importError) {
      addCourseError = importError instanceof Error
        ? importError.message
  ...
  function moveCourse(courseId: string, targetSemester: string, targetIndex?: number): void {
    const course = coursesById[courseId];
    if (!course) {
      return;
    }
    ...
    if (!columnCourseIds[sourceSemester] || !columnCourseIds[targetSemester]) {
      return;
    }
    ...
    const destinationIds = sourceSemester === targetSemester ? sourceIds : [...columnCourseIds[targetSemester]];

    let destinationIndex =
      typeof targetIndex === 'number' ? Math.max(0, Math.min(targetIndex, destinationIds.length)) : destinationIds.length;

    if (sourceSemester === targetSemester && typeof targetIndex === 'number' && targetIndex > sourceIndex) {
      destinationIndex -= 1;
    }
  ...
  function handleDropOnCard(targetSemester: string, targetCourseId: string): void {
    if (!draggedCourseId || draggedCourseId === targetCourseId) {
      return;
    }

    const index = columnCourseIds[targetSemester]?.indexOf(targetCourseId);
    moveCourse(draggedCourseId, targetSemester, index >= 0 ? index : undefined);
    handleDragEnd();
  }
  ...
  $: selectedCourseResolved =
    selectedCourseData ?? (selectedCourseId ? (coursesById[selectedCourseId] ?? null) : null);
  ...
  function removeSelectedCourse(): void {
    ...
    if (columnCourseIds[course.semester]) {
      columnCourseIds = {
        ...columnCourseIds,
        [course.semester]: (columnCourseIds[course.semester] ?? []).filter((id) => id !== courseId)
      };
    }
  ...
  onMount(() => {
    ...
    void (async () => {
      try {
        ...
        error = loadError instanceof Error ? loadError.message : 'Failed to load scheduler data.';
      } finally {
        loading = false;
        await tick();
        queueEdgeMeasurement();
...
<div class="page">
  <header class="page-header">
    ...
    <p>Drag courses between semesters and inspect prerequisite health in real time. The default schedule is my own. Course data may not be fully accurate; please report any issues <a href="/contact">here</a>.</p>
  </header>
    ...
    <div class="scheduler-layout">
          ...
          <svg class="edge-layer" aria-hidden="true">
            {#each nonInvalidEdgeSegments as segment}
              <line
                ...
                class:edge-invalid={segment.status === 'invalid'}
                class:edge-concurrent={segment.status === 'concurrent'}
                class:edge-dimmed={segment.dimmed}
                class:edge-highlighted={segment.highlighted}
                class:edge-invalid-highlight={segment.highlighted && segment.status === 'invalid'}
              />
            {/each}
          ...
          <div class="columns">
            {#each semesterOrder as semester (semester)}
              <div
                ...
                class:drop-target={dragOverSemester === semester}
                role="region"
              ...
              >
                <div class="semester-header">
                  <h2>{semester}</h2>
                  <span>{semesterCredits[semester] ?? 0} credits</span>
                </div>
                ...
                <div class="semester-courses">
                  {#each columnCourseIds[semester] ?? [] as courseId (courseId)}
                    ...
                    <button
                      ...
                      class="course-card"
                      class:selected={selectedCourseId === courseId}
                      draggable="true"
                      on:pointerdown={() => setSelectedCourse(courseId, 'pointerdown')}
                      on:focus={() => setSelectedCourse(courseId, 'focus')}
                      on:click={() => setSelectedCourse(courseId, 'click')}
                      on:dragstart={() => handleDragStart(courseId)}
                      on:dragend={handleDragEnd}
                      on:dragover={(event) => event.preventDefault()}
                      on:drop={(event) => {
                        event.preventDefault();
                        handleDropOnCard(semester, courseId);
                      }}
                    >
                      <header>
                        <h3>{course.name}</h3>
      ...
      <aside class="terminal-box details">
        <div class="scheduler-controls">
          <button type="button" on:click={handleAutoDetangle}>Auto-detangle lines</button>
        </div>
        ...
        <div class="terminal-box add-course-box">
          ...
          <datalist id="course-catalog-options">
            {#each catalogSuggestionValues as suggestionValue}
              <option value={suggestionValue} label={suggestionValue}></option>
            {/each}
          </datalist>
          ...
          <button type="button" on:click={addCourseToSemester}>Add</button>
        </div>

        <div class="terminal-box io-box">
          <h3>Import / Export</h3>
          <div class="io-actions">
            <button type="button" on:click={handleExportJson}>Export JSON</button>
            <button type="button" on:click={() => importInputElement?.click()}>Import JSON</button>
            <input
        ...
        <div class="terminal-box selected-course-box">
          {#if selectedCourseResolved}
            ...
            {#if selectedCourseId && typeof requirementProgressByCourseId[selectedCourseId] === 'number'}
              <p>
                Prereq coverage: {Math.round((requirementProgressByCourseId[selectedCourseId] ?? 0) * 100)}%
              </p>
            {/if}

            <div class="touch-controls">
              <button type="button" on:click={() => moveSelectedCourse(-1)}>Move Earlier</button>
              <button type="button" on:click={() => moveSelectedCourse(1)}>Move Later</button>
            </div>

            <button type="button" class="remove-course-button" on:click={removeSelectedCourse}>
              Remove Course
            </button>
                ...
                {#each selectedCourseRequirementGroups as requirement}
                  <li>
                    ...
                    {#if requirement.canBeConcurrent}
                      <span class="req-tag">concurrent allowed</span>
                    {/if}
                  </li>
                {/each}
              </ul>
