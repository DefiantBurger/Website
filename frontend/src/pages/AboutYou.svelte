<script lang="ts">
	...
	import {
		loadRequestContextSnapshot,
		type RequestContextSnapshot,
	} from "../lib/aboutYou/api";
	...
	function collectBrowserFields(): DataField[] {
		...
		return [
			...
			field(
				"Do Not Track",
				nav.doNotTrack || win.doNotTrack || "Not set",
			),
		...
		try {
			...
			requestContextError =
				error instanceof Error
					? error.message
					: "Failed to fetch request context.";
			requestContext = null;
		} finally {
...
<svelte:head>
	...
	<meta
		name="description"
		content="See what your browser and requests reveal during your visit. Read-only, no analytics profile created."
	/>
</svelte:head>
...
<div class="page">
	<PageHeader
		title="About You"
		description="This page shows what your browser and requests can reveal during your visit. It is read-only and does not create a new analytics profile."
	>
		<p class="refresh-time">Snapshot refreshed: {refreshedAt}</p>
		<button type="button" on:click={refreshLocalSnapshot}
			>Refresh Browser Snapshot</button
		>
	</PageHeader>
	...
	<section class="terminal-box section">
		...
		<p class="section-intro">
			These counters are in-memory only and reset when this tab session
			ends.
		</p>
		...
		<article class="field-row">
			...
			<p class="value">
				Current: {interactionSnapshot.currentRoute ?? "None"} | Previous:
				{interactionSnapshot.previousRoute ?? "None"}
			</p>
		</article>
	...
	<section class="terminal-box section">
		...
		{:else if requestContext}
			...
			<article class="field-row">
				...
				<p class="value">
					{showFullIp
						? requestContext.network.clientIp
						: requestContext.network.clientIpMasked}
			...
			<article class="field-row">
				...
				<p class="value">
					{requestContext.network.hasForwardedHeaders ? "Yes" : "No"}
				</p>
			</article>
			...
			<article class="field-row">
				...
				<ul>
					<li>
						User-Agent: {requestContext.headers.userAgent ||
							"Unavailable"}
					</li>
					<li>
						Accept-Language: {requestContext.headers
							.acceptLanguage || "Unavailable"}
					</li>
					<li>
						Referer: {requestContext.headers.referer ||
							"Unavailable"}
					</li>
					<li>
						Do-Not-Track: {requestContext.headers.doNotTrack ||
							"Unavailable"}
					</li>
				</ul>
				<p class="note">
					Snapshot timestamp: {requestContext.timestamp}
				</p>
			</article>
			...
			<article class="field-row">
				...
				{#if requestContext.geolocation.available}
					<ul>
						<li>
							Provider: {requestContext.geolocation.provider ||
								"Unavailable"}
						</li>
						<li>
							Country: {requestContext.geolocation.country ||
								"Unavailable"}
						</li>
						<li>
							Region: {requestContext.geolocation.region ||
								"Unavailable"}
						</li>
						<li>
							City: {requestContext.geolocation.city ||
								"Unavailable"}
						</li>
						<li>
							Postal: {requestContext.geolocation.postal ||
								"Unavailable"}
						</li>
						<li>
						...
						<li>
							Latitude: {requestContext.geolocation.latitude ??
								"Unavailable"}
						</li>
						<li>
							Longitude: {requestContext.geolocation.longitude ??
								"Unavailable"}
						</li>
						<li>
							ASN: {requestContext.geolocation.asn ||
								"Unavailable"}
						</li>
						<li>
							Organization: {requestContext.geolocation.org ||
								"Unavailable"}
						</li>
					</ul>
				{:else}
					<p class="value">
						Unavailable: {requestContext.geolocation.reason ||
		...
		<button type="button" on:click={refreshServerSnapshot}
			>Refresh Request Snapshot</button
		>
	</section>
	...
	<section class="terminal-box section">
		...
		<p class="section-intro">
			The site could technically infer these, but this feature
			intentionally does not store or profile them.
		</p>
		...
		<article class="field-row">
			...
			<p class="value">
				No multi-signal browser fingerprint is generated.
			</p>
		</article>
	</section>
</div>
